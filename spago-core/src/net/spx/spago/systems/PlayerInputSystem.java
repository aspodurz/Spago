...
public class PlayerInputSystem extends EntityProcessingSystem implements
	...
	protected void process(Entity e) {
		...
		// position.x += TrigLUT.cos(angleInRadians) * 500f *
		// world.getDeltaFloat();
		// position.y += TrigLUT.sin(angleInRadians) * 500f *
		// world.getDeltaFloat();
		...
		/*
		 * if(up) { velocity.vectorY =
		 * MathUtils.clamp(velocity.vectorY+(world.getDeltaFloat
		 * ()*VerticalThrusters), -VerticalMaxSpeed, VerticalMaxSpeed); }
		 * if(down) { velocity.vectorY =
		 * MathUtils.clamp(velocity.vectorY-(world.
		 * getDeltaFloat()*VerticalThrusters), -VerticalMaxSpeed,
		 * VerticalMaxSpeed); }
		 * 
		 * if(left) { velocity.vectorX =
		 * MathUtils.clamp(velocity.vectorX-(world.
		 * getDeltaFloat()*HorizontalThrusters), -HorizontalMaxSpeed,
		 * HorizontalMaxSpeed); } if(right) { velocity.vectorX =
		 * MathUtils.clamp(
		 * velocity.vectorX+(world.getDeltaFloat()*HorizontalThrusters),
		 * -HorizontalMaxSpeed, HorizontalMaxSpeed); }
		 */

		if (shoot) {
			if (timeToFire <= 0) {
				EntityFactory.createPlayerBullet(world, position.x - 27,
						position.y + 2).addToWorld();
				EntityFactory.createPlayerBullet(world, position.x + 27,
						position.y + 2).addToWorld();
				timeToFire = FireRate;
			}
