...
import net.spx.spago.components.ScaleAnimation;
import net.spx.spago.components.Sprite;
import net.spx.spago.components.Velocity;
import net.spx.spago.utils.Constants;
...
public class EntityFactory {
	...
	public static Entity createPlayerBullet(World world, float x, float y) {
		...
		world.getManager(GroupManager.class).add(e,
				Constants.Groups.PLAYER_BULLETS);

		return e;
	}

	public static Entity createEnemyShip(World world, String name,
			Sprite.Layer layer, float health, float x, float y,
			float velocityX, float velocityY, float boundsRadius) {
		Entity e = world.createEntity();
	...
	public static Entity createStar(World world) {
		...
		Position position = new Position();
		position.x = MathUtils.random(-ScreenConfiguration.getInstance()
				.getFrameWidth() / 2, ScreenConfiguration.getInstance()
				.getFrameWidth() / 2);
		position.y = MathUtils.random(-ScreenConfiguration.getInstance()
				.getFrameHeight() / 2, ScreenConfiguration.getInstance()
				.getFrameHeight() / 2);
		e.addComponent(position);
